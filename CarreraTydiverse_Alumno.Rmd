...
```{r}
cran2%>%arrange(country,ip_id)%>%arrange(desc(r_version))
```
...
```{r}
avg_bytes<-cran%>%summarize(mean(size))
avg_bytes
```
# Agrupamientos
...
pack_sum<-by_package%>%summarise(count=n(),unique=n_distinct(ip_id),countries=n_distinct(country),med_bytes=median(size))
pack_sum
```
...
```{r}
head(cran)
umbral<-pack_sum%>%quantile(count,probs=0.99,na.rm=T)
top_counts<-
```
